...
<script type="text/javascript">
  ...
  function goPage( pageSize, url) {
    var currentPage = $("#input_page").val();
    if(currentPage == null || currentPage == '') {
      currentPage = 0;
    }
    if(currentPage > 0) {
      changePage(currentPage, 10, url);
    }
...
<div class="pagin">
  ...
  <ul class="paginList">
    <c:if test="${page.currentPage > 1}">
      <li class="paginItem"><a href="javascript:;" onclick="changePage(${page.currentPage - 1}, ${page.showCount}, '${param.url}')">
    ...
    <c:if test="${page.totalPage <= 8}">
      <c:forEach var="i" begin="1" end="${page.totalPage}">
        ...
        <c:if test="${page.currentPage != i}">
          <li class="paginItem"><a href="javascript:;" onclick="changePage(${i}, ${page.showCount}, '${param.url}')">${i}</a></li>
        </c:if>
      </c:forEach>
    </c:if>
    <c:if test="${page.totalPage > 8}">
      ...
      <c:if test="${page.currentPage > 5}">
        <!-- 默认显示第一页、第二页 -->
        <li class="paginItem"><a href="javascript:;" onclick="changePage(1, ${page.showCount}, '${param.url}')">1</a></li>
        <li class="paginItem"><a href="javascript:;" onclick="changePage(2, ${page.showCount}, '${param.url}')">2</a></li>
    ...
    <c:if test="${page.currentPage < page.totalPage}">
      <li class="paginItem"><a href="javascript:;" onclick="changePage(${page.currentPage + 1}, ${page.showCount}, '${param.url}')"><span class="pagenxt"></span></a></li>
    </c:if>
    <c:if test="${page.currentPage >= page.totalPage}">
      <li class="paginItem"><a href="javascript:;"><span class="pagenxt"></span></a></li>
    </c:if>
  </ul>
  <span class="page_span">到<input type="text" id="input_page" name="input_page" onkeyup="pagework();" style="border:1px solid #ccc; width:30px; height:20px;padding-left:5px; line-height:10px; margin:0px 10px;" />页<input type="button" class="page_commit" onclick="goPage(${page.showCount},'${param.url}');" value="确定"></span>
